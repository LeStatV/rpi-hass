...
runSupervisor() {
    ...
    /usr/bin/docker rm --force hassio_supervisor >/dev/null || true
    /usr/bin/docker run --name hassio_supervisor \
        --privileged \
        $APPARMOR \
...
start_hassio() {
    ...
    HASSIO_IMAGE_ID=$(/usr/bin/docker inspect --format='{{.Id}}' ${SUPERVISOR})
    HASSIO_CONTAINER_ID=$(/usr/bin/docker inspect --format='{{.Image}}' hassio_supervisor || echo "--")
    ...
    # Run supervisor
    ([ "${HASSIO_IMAGE_ID}" = "${HASSIO_CONTAINER_ID}" ] && /usr/bin/docker start --attach hassio_supervisor) || runSupervisor
}
